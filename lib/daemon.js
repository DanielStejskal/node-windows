          ...
          me.execute('"'+path.resolve(me.root,me._exe)+'" uninstall',function(error,stdout,stderr){
            if (error){
              ...
              */
              ...
              const regex = new RegExp('^(?:' + me.id +'\.)(?:(?:(?:wrapper|out|err)\.log)|(?:exe|xml|exe\.config))$', 'i');
              // Remove only given service files
              var _other_files = fs.readdirSync(me.root).filter(function (file) {
                return regex.test(file)
              }).map(f => rm(f));

              // Remove the dir if it's empty
              if (fs.readdirSync(me.root).filter(fn => fn !== '.' && fn !== '..').length === 0) {
